...
|depends|Resources depended by this task additionally. The resources are used only to decide whether the corresponding output files are up to date.| No; can be appear arbitrary number of times
...
The way to configure this element is the same as that to configure one that appears in _Chionographis_ driver. So please see the _Chionographis_ driver's explanation for details.| No; can be appear arbitrary number of times
|transform|A _Transform_ sink. .4+| No; at least one sink required
|all|An _All_ sink.
|snip|A _Snip_ sink.
|output|An _Output_ sink.
