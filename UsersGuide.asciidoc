...
This target does following:
 ...
 . then finds all fragments from the resulted document which matches XPath criteria `/genera/genus`,
...
[options="header", cols="1,3,1"]
|=================
| Attribute / Child | Description | Required
...
You can tell _Chionographis_ and _Transform_ drivers to refer the new-ness these depended files by _depends_ child elements.
