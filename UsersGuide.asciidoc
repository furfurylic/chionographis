...
|failOnNonfatalError|Whether nonfatal errors should make the build fail. A nonfatal error is an error attributed to one input source and not likely to affect processing of other input sources. | No; defaults to +no+
...
|_Text_|The value of the stylesheet parameter. | Yes
|=================
