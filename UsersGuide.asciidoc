...
|namespace|A pair of a namespace prefix and a namespace name which is mapped from the prefix. This mapping is used by descentant _Transform_, _All_ and _Snip_ drivers.| No; can appear arbitrary number of times
...
|Element: _resource collection_|The referenced resources represented by Ant resource collections. | Yes if there are no nested dependency; can appear arbitrary number of times greater or equal to one
|Element: _depends_|Nested dependency. | No; can appear arbitrary number of times
|=================
